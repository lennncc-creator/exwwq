...
TOKEN = "7222834007:AAHf5tzHepsvfvcU4lPLCpZRDO6h0ik66to"
ADMIN_IDS = {7700558902, 1154554868, 1190117295, 7701801337}
BOT_USERNAME = "ExodusRBX_bot"
...
async def get_user_subscriptions(user_id: int, links: list[str] | None = None, start_date: str | None = None, end_date: str | None = None):
    """
    Проверяем подписку пользователя через SubGram API.
    Возвращает кортеж: (status, code, sponsors)
    """
    headers = {
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{SUBGRAM_BASE_URL}/get-user-subscriptions", headers=headers, json=data) as resp:
                if not resp.ok:
                    logging.error(f"SubGram get_user_subscriptions error: {await resp.text()}")
                    return "error", resp.status, []

                res = await resp.json()
                sponsors = res.get("additional", {}).get("sponsors", [])
                return res.get("status"), res.get("code"), sponsors

    except Exception as e:
        logging.error(f"SubGram get_user_subscriptions exception: {e}")
        return "error", 400, []



async def request_op(message_or_call, gender: str | None = None):
    """
    Получаем блок обязательной подписки для пользователя.
    Универсально для Message или CallbackQuery.
    """
    headers = {
        "Content-Type": "application/json",
        "Auth": SUBGRAM_API_KEY,
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{SUBGRAM_BASE_URL}/request-op/", headers=headers, json=data) as resp:
                if not resp.ok:
                    logging.error(f"SubGram request_op error: {await resp.text()}")
                    return "error", resp.status, []

                res = await resp.json()
                links = res.get("links", [])
                return res.get("status"), res.get("code"), links

    except Exception as e:
        logging.error(f"SubGram request_op exception: {e}")
        return "error", 400, []
...
async def show_subscribe_block(message):
    status, code, links = await request_op(message)
    ...
    if status == "ok" and code == 200:
        ...
        inline_keyboard = [[InlineKeyboardButton(text="Подписаться", url=link)] for link in links]
        # Добавляем кнопку "Проверить подписку"
        inline_keyboard.append([InlineKeyboardButton(text="✅ Проверить подписку", callback_data="subgram-op")])

        # Передаем в InlineKeyboardMarkup через inline_keyboard
        kb = InlineKeyboardMarkup(inline_keyboard=inline_keyboard)

        await message.answer("Для продолжения подпишись на спонсоров:", reply_markup=kb)
        return False
    else:
        await message.answer("Ошибка сервиса SubGram, попробуйте позже")
        return False
...
async def ensure_subscribed(
    ...
    Если пользователь не подписан — выводит клавиатуру подписки с фото и текстом.
    """
    user_id = message.from_user.id
    bot = message.bot if isinstance(message, Message) else message.message.bot

    # 🔹 Получаем блок обязательной подписки
    status, code, links = await request_op(message)
    ...
    if status == "ok" and code == 200:
        ...
        if isinstance(message, Message):
            await message.answer_photo(photo=IMAGES["subscription"], caption=text, reply_markup=kb)
        else:
            await message.message.answer_photo(photo=IMAGES["subscription"], caption=text, reply_markup=kb)

        return False
    else:
        # Ошибка сервиса
        err_text = "Ошибка сервиса SubGram, попробуйте позже"
        if isinstance(message, Message):
            await message.answer(err_text)
        else:
            await message.message.answer(err_text)
        return False
...
async def subgram_callback_query(call: CallbackQuery):
    # 1. Сразу отвечаем на callback, чтобы Telegram не ругался
    # Пустой ответ или alert=True/False
    try:
        await call.answer()
    except Exception:
        pass  # если уже устарел, просто игнорируем

    user_id = call.from_user.id

    if not call.message:
        return  # сообщение исчезло, выходим

    if call.data == "subgram-op":
        status, code, sponsors = await get_user_subscriptions(user_id)
        unsubscribed = [s for s in sponsors if s.get("status") != "subscribed"]

        if not unsubscribed:
            await call.message.answer("Вы подписаны на все ресурсы 👍")
            return

        # Формируем InlineKeyboard корректно
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=s.get("resource_name", "Подписаться"),
                        url=s.get("link", "#")
                    )
                ] for s in unsubscribed
            ] + [[InlineKeyboardButton(text="✅ Проверить снова", callback_data="subgram-op")]]
        )

        await call.message.answer("Вы не подписаны на все ресурсы:", reply_markup=kb)
...
async def cmd_start(m: Message, state: FSMContext):
    ...
    # показать конкретную карточку или главное меню
    if target:
        await open_script_card(user_id, m, target[0], target[1])
    else:
        await show_main_menu(user_id, m)
...
async def show_main_menu(user_id: int, where: Message | CallbackQuery):
    ...
    if isinstance(where, Message):
        ...
        await where.answer()  # обязательно ответить callback, чтобы убрать таймаут
...
async def cb_subs_check(c: CallbackQuery, state: FSMContext):
    ...
    if pending:
        await open_script_card(c.from_user.id, c, pending["cat"], pending["script"])
